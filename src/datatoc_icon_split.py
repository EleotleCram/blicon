...
def write_subimage(sub_x, sub_y, sub_w, sub_h,
    ...
    # print("Writing: %s (x=%d y=%d w=%d h=%d)" % (filepath, sub_x, sub_y, sub_w, sub_h))
    image_map_x1 = sub_x
    image_map_y2 = 1280 - sub_y
    image_map_x2 = sub_x + sub_w
    image_map_y1 = image_map_y2 - sub_h
    print('''<area shape="rect" coords="%d,%d,%d,%d" onmouseover="writeText('%s')" onmouseout="writeText('&nbsp;')" href="#" alt="%s" />''' %
        (image_map_x1, image_map_y1, image_map_x2, image_map_y2, icon_c_name, icon_c_name))
...
def dice_icon_name(
    ...
    """
    if name_style.startswith('UI_ICONS'):
        ...
        if not _dice_icon_name_cache:
            ...
            ui_icons_h = os.path.join(SOURCE_DIR, "UI_icons.h")
        ...
        elif name_style == 'UI_ICONS_C_NAME':
            id_str = "ICON_%s" % icon_name.upper()
...
def dice(
    ...
    for x in range(parts_x):
        for y in range(parts_y):
            ...
            icon_c_name = dice_icon_name(
                x, y,
                parts_x, parts_y,
                name_style="UI_ICONS_C_NAME", prefix=output_prefix
            )
            if not id_str:
                continue
